...
public class RCTCamera {
  ...
  public CamcorderProfile setCaptureVideoQuality(int cameraType, String captureQuality) {
    ...
    int cameraId = _cameraTypeToIndex.get(cameraType);
    int quality;
    switch (captureQuality) {
      case RCTCameraModule.RCT_CAMERA_CAPTURE_QUALITY_LOW:
      ...
      default:
        videoSize = getSmallestSize(supportedSizes);
        quality = Integer.parseInt(captureQuality);
    }
    if (!CamcorderProfile.hasProfile(cameraId, quality)) {
      for (int i = CamcorderProfile.QUALITY_480P; i <= CamcorderProfile.QUALITY_2160P; i++) {
        if (CamcorderProfile.hasProfile(cameraId, i)) {
          cm = CamcorderProfile.get(cameraId, i);
          break;
        }
      }

    } else {
      cm = CamcorderProfile.get(cameraId, quality);
    }
